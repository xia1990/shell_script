...
function do_snapshot(){
	...
	readarray -t xml_array < test.xml
	for line in "${xml_array[@]}"
	do
		echo $line | grep -aoe "<project" >/dev/null
		if [ "$?" == 0 ];then
			fetch_name=$(grep -aoe "fetch=[.a-z_A-Z0-9:\"/-]*" test.xml | awk -F'"' '{print $2}')
			echo $fetch_name
			#默认的分支名称
			default_revision=$( grep -aoe "revision=[.a-z_A-Z0-9\"/-]*" test.xml | awk -F'"' '{print $2}')
			...
			#得到每个仓库的最新的commitID
			commitID=$(git ls-remote $fetch_name$project_name -b $branch_name | awk '{print $1}')
