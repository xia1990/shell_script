...
function arg_parse(){
	if [ "$PROJECTAP" != "WI1401Q_AP_DEV" ];then
		echo "AP项目名称error"
		exit
	elif [ "$PROJECTBP" != "Wi1401Q_BP" ];then
		echo "BP项目名称error"
		exit
	elif [ "$BRANCHAP" != "wi1401q_main_r2539dev_161118" ];then
		echo "AP分支名称错误!"
		exit
	elif [ "$BRANCHBP" != "wi1401q_r2539dev" ];then
		echo "BP分支名称错误!"
		exit
	elif [ "$FTP_PATH" != "./Qualcomm/8039/Wi1401Q/Wi1401Q_Main_branch" ];then
		echo "FTP路径错误！"
		exit
	else
		echo ""
