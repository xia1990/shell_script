		...
		do
			...
			if [ "$?" == 0 ];then
				...
				if [ -z "$revision_line" ];then
					...
					#将分支名称输出到一个文件中
					echo $line | grep -aoe "revision=[.A-Z_a-z0-9\"/-]*" | awk -F'"' '{print $2}' >> project_revision.txt
				fi
			fi
		done
	popd
...
function do_snapshot(){
	pushd ${PATHROOT}/.repo/manifests
		...
		do
			...
			if [ "$?" == 0 ];then
				revision=$(echo $m | grep -aoe "revision=[.A-Z_a-z0-9\"/-]*")
				#如果没有revision
				if [ -z "$revision" ];then
					name=$(echo $m | grep -aoe "name=[.A-Z_a-z0-9\"/-]*"  | awk -F'"' '{print $2}')
					echo $name
					...
					echo $m | sed -n "s/name=[.A-Z_a-z0-9\"/-]*/& revision=\"$commitID\"/g"p | sed -n "s/revision=[.A-Z_a-z0-9\"/-]*/& upstream=\"$branch_name\"/"p | tee -a local.xml
				else
					name=$(echo $m | grep -aoe "name=[.A-Z_a-z0-9\"/-]*"  | awk -F'"' '{print $2}')
					echo $name
					branch_name=$(grep -w "$name" info.txt | awk '{print $NF}' | awk -F'/' '{print $NF}')
					...
					echo $m | sed -n "s/revision=[.A-Z_a-z0-9\"/-]*/revision=\"$commitID\"/g"p | sed -n "s/revision=[.A-Z_a-z0-9\"/-]*/& upstream=\"$branch_name\"/"p | tee -a local.xml
				fi
			else
