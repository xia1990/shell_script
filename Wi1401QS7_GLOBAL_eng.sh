...
function building(){
	...
	echo "target=msm8909-global-eng" >> build_target.cfg
...
function packing(){
	...
	pushd ${PATHROOT}/$PROJECTAP/LINUX/android/build/tools
		...
		echo $VERSION
		Target_name=SUB7_GLOBAL_S${VERSION}_${date}
		MVersion=${VERSION}_eng
		Pack_name=SUB7_GLOBAL_${MVersion}_${date}
		echo ${Pack_name}
	popd
}
...
function make_zipfile(){
	packing
	pushd ${PATHROOT}/$PROJECTBP/SCM_COPY_FILES/msm8909_global_eng
	...
	popd
	pushd ${PATHROOT}/$PROJECTBP/SCM_COPY_FILES/msm8909_global_eng/multiflash_images
...
function ftp_upload(){
  ...
  mkdir ENG
  cd ENG
  lcd ${PATHROOT}/$PROJECTBP/SCM_COPY_FILES/msm8909_global_eng
  put ${Pack_name}_OriginalFactory.zip
  put DEBUG_INFO.zip
  put ${Pack_name}_modem_image.zip
  lcd ${PATHROOT}/$PROJECTBP/SCM_COPY_FILES/msm8909_global_eng/multiflash_images
  put ${Pack_name}_image.zip
  mkdir sd
  cd sd
  lcd ${PATHROOT}/$PROJECTBP/SCM_COPY_FILES/msm8909_global_eng/sd
  put msm8909-ota-eng.yinjigang.zip
  lcd ${PATHROOT}/$PROJECTBP/SCM_COPY_FILES/msm8909_global_eng/sd
  put msm8909-target_files-eng.yinjigang.zip
  bye
EOC
