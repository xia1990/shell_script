...
function building(){
	...
	echo "target=msm8909-local-eng" >> build_target.cfg
...
function make_zipfile(){
	packing
	pushd ${PATHROOT}/$PROJECTBP/SCM_COPY_FILES/msm8909_local_eng
	...
	popd
	pushd ${PATHROOT}/$PROJECTBP/SCM_COPY_FILES/msm8909_local_eng/multiflash_images
...
function ftp_upload(){
  ...
  mkdir ENG
  cd ENG
  lcd ${PATHROOT}/$PROJECTBP/SCM_COPY_FILES/msm8909_local_eng
  put ${Pack_name}_OriginalFactory.zip
  put DEBUG_INFO.zip
  put ${Pack_name}_modem_image.zip
  lcd ${PATHROOT}/$PROJECTBP/SCM_COPY_FILES/msm8909_local_eng/multiflash_images
  put ${Pack_name}_image.zip
  mkdir sd
  cd sd
  lcd ${PATHROOT}/$PROJECTBP/SCM_COPY_FILES/msm8909_local_eng/sd
