...
function building(){
	...
	echo "target=msm8916_64-t1sub14-local-eng" > build_target.cfg
...
function make_zipfile(){
	packing
	echo "Ready to pack"
	pushd "$PATHROOT"/$PROJECTBP/SCM_COPY_FILES/msm8916_64_t1sub14_local_eng
	...
	popd
	pushd "$PATHROOT"/$PROJECTBP/SCM_COPY_FILES/msm8916_64_t1sub14_local_eng/multiflash_images
...
function ftp_upload(){
  ...
  mkdir ENG
  cd ENG
  lcd "$PATHROOT"/$PROJECTBP/SCM_COPY_FILES/msm8916_64_t1sub14_local_eng
  ...
  mkdir sd
  cd sd
  lcd "$PATHROOT"/$PROJECTBP/SCM_COPY_FILES/msm8916_64_t1sub14_local_eng/sd
