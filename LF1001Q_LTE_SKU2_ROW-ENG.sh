...
PATHROOT=$(pwd)
project=LF1001Q
git_branch=Basil_RU_MP
Date=`date +%Y%m%d`
Ftp_PATCH="./Qualcomm/8976/LF1001Q/RU_MP/"
MOD_VER=false
LF1001Q_LTE_SKU2_ROW_ENG=true
...
function modify_version(){
	if [ ${MOD_VER} = "true" ] ; then
  	pushd ${PATHROOT}/${project}/LINUX/android/build/tools
  		...
  		git diff
  		git add ${PATHROOT}/${project}/LINUX/android/build/tools/lenovo_buildinfo.mk
...
function build_code(){
	pushd ${PATHROOT}/${project}/
		...
		if [ ${LF1001Q_LTE_SKU2_ROW_ENG} = "true" ];then
  			echo "target=msm8952_64-lte-row-sku2-eng">>build_target.cfg
		else
  			echo  ""
		fi
...
function make_zipfile(){
	packing
	if [ ${LF1001Q_LTE_SKU2_ROW_ENG} = "true" ] ; then
		pushd ${PATHROOT}/${project}/SCM_COPY_FILES/msm8952_64_lte_row_sku2_eng
		...
		popd
  		pushd ${PATHROOT}/${project}/SCM_COPY_FILES/msm8952_64_lte_row_sku2_eng/multiflash_images
  			echo LTE_SKU2_ROW > version.txt
  			echo ${EX_version} >> version.txt
  			echo ${Pack_name} >> version.txt
  			zip -rq ${Pack_name}.zip ./*
		popd
  		pushd ${PATHROOT}/${project}/SCM_COPY_FILES/msm8952_64_lte_row_sku2_eng/sd
...
ftp -n 10.30.11.100 2>&1 <<EOC
  ...
  mkdir target
  cd target
  lcd ${PATHROOT}/${project}/SCM_COPY_FILES/msm8952_64_lte_row_sku2_eng
  mkdir LTE_SKU2_ROW-ENG
  cd LTE_SKU2_ROW-ENG
  put ${Pack_name}_OriginalFactory.zip
  put DEBUG_INFO.zip
  lcd ${PATHROOT}/${project}/SCM_COPY_FILES/msm8952_64_lte_row_sku2_eng/multiflash_images
  put ${Pack_name}.zip
  mkdir sd
  cd sd
  lcd ${PATHROOT}/${project}/SCM_COPY_FILES/msm8952_64_lte_row_sku2_eng/sd
