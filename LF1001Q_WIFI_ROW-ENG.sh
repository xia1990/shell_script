...
function build_code(){
	pushd ${PATHROOT}/${project}/
		...
		if [ ${LF1001Q_WIFI_ROW_ENG} = "true" ];then
  			echo "target=msm8952_64-wifi-row-none-eng">>build_target.cfg
		else
  			echo  ""
		fi
...
function make_zipfile(){
	packing
	if [ ${LF1001Q_WIFI_ROW_ENG} = "true" ] ; then
		pushd ${PATHROOT}/${project}/SCM_COPY_FILES/msm8952_64_wifi_row_none_eng
		...
		popd
  		pushd ${PATHROOT}/${project}/SCM_COPY_FILES/msm8952_64_wifi_row_none_eng/multiflash_images
  			echo WIFI_ROW > version.txt
  			echo ${EX_version} >> version.txt
  			echo ${Pack_name} >> version.txt
  			zip -rq ${Pack_name}.zip ./*
		popd
  		pushd ${PATHROOT}/${project}/SCM_COPY_FILES/msm8952_64_wifi_row_none_eng/sd
...
ftp -n 10.30.11.100 2>&1 <<EOC
  ...
  mkdir target
  cd target
  lcd ${PATHROOT}/${project}/SCM_COPY_FILES/msm8952_64_wifi_row_none_eng
  mkdir WIFI_ROW-ENG
  cd WIFI_ROW-ENG
  put ${Pack_name}_OriginalFactory.zip
  put DEBUG_INFO.zip
  lcd ${PATHROOT}/${project}/SCM_COPY_FILES/msm8952_64_wifi_row_none_eng/multiflash_images
  put ${Pack_name}.zip
  mkdir sd
  cd sd
  lcd ${PATHROOT}/${project}/SCM_COPY_FILES/msm8952_64_wifi_row_none_eng/sd
