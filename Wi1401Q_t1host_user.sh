...
FTP_PATH="./Qualcomm/8039/Wi1401Q/"
M_Version=false
...
function building(){
	...
	echo "target=msm8916_64-t1host-local-user" > build_target.cfg
...
function packing(){
	...
	Pack_name=Wi1401q_MAIN_t1host_local_${MVersion}_${DATE}

	#Target_name=Wi1401q_S${Old_VER_NUM}_t1host_${DATE}
	#Pack_name=Wi1401q_t1host_local_${MVersion}_${DATE}
	echo ${Pack_name}
	popd
}
...
function make_zipfile(){
	packing
	echo "Ready to pack"
	pushd "$PATHROOT"/$PROJECTBP/SCM_COPY_FILES/msm8916_64_t1host_local_user
	...
	popd
	pushd "$PATHROOT"/$PROJECTBP/SCM_COPY_FILES/msm8916_64_t1host_local_user/multiflash_images
...
function ftp_upload(){
  ...
  mkdir USER
  cd USER
  lcd "$PATHROOT"/$PROJECTBP/SCM_COPY_FILES/msm8916_64_t1host_local_user
  ...
  mkdir sd
  cd sd
  lcd "$PATHROOT"/$PROJECTBP/SCM_COPY_FILES/msm8916_64_t1host_local_user/sd
