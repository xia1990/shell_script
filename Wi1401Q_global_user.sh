...
function building(){
	...
	echo "target=msm8916_64-t1host-global-user" > build_target.cfg
...
function make_zipfile(){
	packing
	echo "Ready to pack"
	pushd "$PATHROOT"/$PROJECTBP/SCM_COPY_FILES/msm8916_64_t1host_global_user
	...
	popd
	pushd "$PATHROOT"/$PROJECTBP/SCM_COPY_FILES/msm8916_64_t1host_global_user/multiflash_images
...
function ftp_upload(){
  ...
  mkdir USER
  cd USER
  lcd "$PATHROOT"/$PROJECTBP/SCM_COPY_FILES/msm8916_64_t1host_global_user
  ...
  mkdir sd
  cd sd
  lcd "$PATHROOT"/$PROJECTBP/SCM_COPY_FILES/msm8916_64_t1host_global_user/sd
