...
function building(){
	...
	echo "target=msm8909-global-user" >> build_target.cfg
...
function make_zipfile(){
	packing
	pushd ${PATHROOT}/$PROJECTBP/SCM_COPY_FILES/msm8909_global_user
	...
	popd
	pushd ${PATHROOT}/$PROJECTBP/SCM_COPY_FILES/msm8909_global_user/multiflash_images
...
function ftp_upload(){
  ...
  mkdir USER
  cd USER
  lcd ${PATHROOT}/$PROJECTBP/SCM_COPY_FILES/msm8909_global_user
  put ${Pack_name}_OriginalFactory.zip
  put DEBUG_INFO.zip
  put ${Pack_name}_modem_image.zip
  lcd ${PATHROOT}/$PROJECTBP/SCM_COPY_FILES/msm8909_global_user/multiflash_images
  put ${Pack_name}_image.zip
  mkdir sd
  cd sd
  lcd ${PATHROOT}/$PROJECTBP/SCM_COPY_FILES/msm8909_global_user/sd
