...
function build_code(){
	pushd ${PATHROOT}/${project}/
		...
		if [ ${LF1001QN_WIFI_PRC_USER} = "true" ];then
  			echo "target=msm8952_64-wifi-prc-none-user">>build_target.cfg
		else
  			echo  ""
		fi
...
function make_zipfile(){
	packing
	if [ ${LF1001QN_WIFI_PRC_USER} = "true" ] ; then
		pushd ${PATHROOT}/${project}/SCM_COPY_FILES/msm8952_64_wifi_prc_none_user
		...
		popd
  		pushd ${PATHROOT}/${project}/SCM_COPY_FILES/msm8952_64_wifi_prc_none_user/multiflash_images
  			echo WIFI_PRC>version.txt
  			echo ${EX_version} >> version.txt
  			echo ${Pack_name} >> version.txt
  			zip -rq ${Pack_name}.zip ./*
		popd
  		pushd ${PATHROOT}/${project}/SCM_COPY_FILES/msm8952_64_wifi_prc_none_user/sd
...
ftp -n 10.30.11.100 2>&1 <<EOC
  ...
  mkdir target
  cd target
  lcd ${PATHROOT}/${project}/SCM_COPY_FILES/msm8952_64_wifi_prc_none_user
  mkdir WIFI_PRC-USER
  cd WIFI_PRC-USER
  put ${Pack_name}_OriginalFactory.zip
  put DEBUG_INFO.zip
  lcd ${PATHROOT}/${project}/SCM_COPY_FILES/msm8952_64_wifi_prc_none_user/multiflash_images
  put ${Pack_name}.zip
  mkdir sd
  cd sd
  lcd ${PATHROOT}/${project}/SCM_COPY_FILES/msm8952_64_wifi_prc_none_user/sd
...
pushd ~
	./mk_userdata_N_WIFI_PRC-USER.sh
popd
